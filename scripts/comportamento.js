...
function mostraConfiguracaoJogo() {
  ...
  document.getElementById(TD_MAXIMO_ALEATORIO).innerHTML = configuracao.maximoAleatorio;
}
...
function pedeMinimoAleatorio() {
  ...
  configuracao.minimoAleatorio = pedeNumeroInteiro(1, configuracao.maximoAleatorio, PROPOSITO_MINIMO_ALEATORIO);
 ...
 * pelos argumentos atribuídos aos dois primeiros parâmetros da função. O
 * pedido é feito as vezes necessárias até ser obtido um valor válido.
 * 
...
function pedeNumeroInteiro(minimo, maximo, proposito = PROPOSITO_ADIVINHAR) {
  ...
  // Exercício: Colocar aqui o código da função.
  let sujeito = parseInt(prompt(proposito + ", digite um número inteiro entre " + minimo + " e " + maximo + "."));
  
  if (Number.isNaN(sujeito) === true) {
    alert("O valor " + sujeito + " não é um número inteiro.");
    sujeito = parseInt(prompt(proposito + ", digite um número inteiro entre " + minimo + " e " + maximo + "."));
  } else if (sujeito < minimo || sujeito > maximo) {
    alert("O valor " + sujeito + " está fora do intervalo de validade.");
    sujeito = parseInt(prompt(proposito + ", digite um número inteiro entre " + minimo + " e " + maximo + "."));
  } else {
    return sujeito;
  }
...
function geraNumeroInteiroAleatorio(minimo = MINIMO_ALEATORIO_OMISSAO,
  ...
  // Exercício: Colocar aqui o código da função.
  const thenumb = Math.floor(Math.random(minimo, maximo)*100);
  return thenumb;
}
